...
+ 在自信息量（self-information）\\(I(x) = \log (\frac{1}{p(x)}) = -\log p(x)\\)的定义中，log对数操作既可以以2为底，也可以以e为底，但它们没有本质区别，只差一个固定的倍数。当讨论信息编码的时候，采用以2为底比较方便，这时候\\(I(x)\\)的单位是bit；而讨论机器学习的时候，以e为底比较方便。.smaller[(source: http://zhangtielei.com/posts/blog-deep-learning-foundation.html)]
